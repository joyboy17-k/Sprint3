...
class ConsoleUI:
    ...
    def __init__(self, netz: UbahnNetz):
        self.netz = netz
    ...
    def choose_start_station(self, ziel: Optional[str] = None) -> str:
        ...
        Returns:
            Der Name der gewählten Station
        """
        print("\n📍 Verfügbare Stationen:")
        stationen = self.netz.stationen_reihenfolge

        for i, station in enumerate(stationen, 1):
            print(f"  {i}. {station}")
        ...
        while True:
            try:
                ...
                if auswahl.isdigit():
                    index = int(auswahl) - 1
                    if 0 <= index < len(stationen):
                        station = stationen[index]
                        if ziel and station.upper() == ziel.upper():
                            print("❌ Start- und Zielstation dürfen nicht identisch sein.")
                            continue
                        return station
                    else:
                        print(f"❌ Bitte wählen Sie eine Nummer zwischen 1 und {len(stationen)}.")
                        continue

                # Versuche als Name
                station_upper = auswahl.upper()
                if station_upper in stationen:
    ...
    def choose_destination_station(self, start: str) -> str:
        ...
        Returns:
            Der Name der gewählten Zielstation
        """
